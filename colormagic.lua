...
local shl, shr, bor, xor, bnot, band, extr = bit32.lshift, bit32.rshift, bit32.bor, bit32.bxor,
    bit32.bnot, bit32.band, bit32.extract
...
function exported_api.load_avatar_tex(name, suppressWarn)
    ...
    if not texture_raw then
        error [[
    Can't load the specified texture from the avatar NBT.
    If this texture is generated by a script, it's not possible to load it into the library this way.
    If you have the raw texture data in a string, pass the string to this function instead.
    Otherwise, make sure the texture is attached to a Blockbench model somewhere - otherwise, it won't be included in your avatar at all.
    ]]
    end

    if host:isHost() and not suppressWarn then
        printJson(toJson {
            {
                text = ("-"):rep(53) .. "\n",
                color = "gray",
            ...
            {
                text=("Warning while loading %s:\n"):format(name),
                color="gold",
            },
            {
                text="Loading a texture into colormagic from a Texture object is inefficient.\n",
                color="yellow",
                underlined=true,
            },
            {
                text="If you're not using the texture elsewhere, consider using a Resource or raw string.\n",
                color="yellow",
            },
            {
                text="Also, any modifications to the texture you made with a script are ignored when importing.\n",
                color="yellow",
            },
            {
                text="[Hover for details & instruction counts]\n",
                color="#ff80ff",
                italic=true,
                hoverEvent={
                    action="show_text",
                    contents={
                        "In Avatar NBT, images are stored as lists of bytes (numbers).\n",
                        "This would be fine if they were unsigned bytes (0 to 255), which can be loaded in bulk with\n",
                        "the Lua builtin 'string.char'. However, the NBT stores SIGNED bytes... which means that looping\n",
                        "over the bytes is required. This will cost 7 instructions per byte in your image.\n",
                        {
                            text="On this " .. #texture_raw .. " byte image, that's " .. (7 * #texture_raw) .. " instructions.",
                            color="yellow",
                        }
                    }
                }
            ...
            {
                text="To supress this warning, pass 'true' as a second argument to load().\n",
                color="#40b0b0",
                hoverEvent={
                    action="show_text",
                    contents={
                        "or load_avatar_tex(), I don't know which you're using"
                    }
                }
            },
            {
...
---@alias colormagic.InFormat string | InputStream | Texture

---@type {[string]: fun(in: colormagic.InFormat, ...: any): colormagic.file_pack}
local loaders = {
    ["string"] = preparse_png,
    ---@param is InputStream
...
---@param input colormagic.InFormat
---@param ... any
